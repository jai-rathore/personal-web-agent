...
func main() {
	...
	emailService := feedback.NewSMTPEmailService(cfg)

	// Initialize handlers
	chatHandler := chat.NewHandler(geminiProvider, contentLoader, guardrailsPipe, cfg)
	feedbackHandler := feedback.NewHandler(cfg, emailService)

	// Initialize HTTP server
	webServer := server.NewServer(cfg, contentLoader, chatHandler, feedbackHandler)
